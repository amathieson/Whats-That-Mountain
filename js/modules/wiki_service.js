...
async function pull_data(poi) {
    if (poi?.tags?.wikidata) {
        ...
        if (wikidata.sitelinks && wikidata.sitelinks.enwiki)
            wikipage = await (await fetch(`https://en.wikipedia.org/w/api.php?format=json&action=query&prop=extracts|pageimages&exintro&explaintext&redirects=1&pilicense=free&piprop=original|name&titles=${wikidata.sitelinks.enwiki.title}`)).json()
